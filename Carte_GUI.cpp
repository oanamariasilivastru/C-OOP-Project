...
void CarteGUI::initializeGUIComponents() {
...
void CarteGUI::reloadDynamicButtons() {
...
void CarteGUI::clearLayout(QLayout* layout) {
	if (layout == NULL)
		return;
	QLayoutItem* item;
...
void CarteGUI::reloadCarteList(vector<Carte> carti) {
...
void CarteGUI::connectSignalsSlots() {
	
	QObject::connect(buttonAddCarte, &QPushButton::clicked, this, &CarteGUI::guiAddCarte);
	
	QObject::connect(buttonDeleteCarte, &QPushButton::clicked, this, &CarteGUI::guiDeleteCarte);

	QObject::connect(buttonModifyCarte, &QPushButton::clicked, this, &CarteGUI::guiModifyCarte);

	QObject::connect(buttonUndo, &QPushButton::clicked, this, &CarteGUI::guiUndoCarte);
	...
	this->reloadCarteLab(srv.filtrareTitlu(filterC));
		});
...
void CarteGUI::guiAddCarte() {
	...
	try {
		...
		this->reloadCarteLab(this->srv.getAll());
		reloadDynamicButtons();
...
void CarteGUI::reloadCarteLab(vector<Carte> carti) {
	list->clear();
	for (auto& carte : carti) {
		string str;
		str.append(carte.getTitlu());
		str.append(" ");
		str.append(carte.getAutor());
		str.append(" ");
		str.append(carte.getGen());
		str.append(" ");
		str.append(std::to_string(carte.getAn()));
